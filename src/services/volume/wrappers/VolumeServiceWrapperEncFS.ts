 ...
 */
export default class VolumeServiceWrapperEncFS
  implements VolumeServiceWrapperInterface {
  ...
  public getUNmountCommand(volume: Volume): string {
    ...
    switch (os.platform()) {
      ...
      default:
        throw new ServiceError(ErrorType.UnsupportedOS);
    }
  ...
  public getMountCommand(volume: Volume, password: String): string {
    ...
    switch (os.platform()) {
      ...
      case "linux":
        ...
        if (volume.ttl > 0) command = command + ` --idle ${volume.ttl}`;
